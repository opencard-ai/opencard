...
| Rotate Upstash token | ✅ Done 2026-05-04 — owner reset the Upstash REST password, all 5 derived env vars (`UPSTASH_KV_REST_API_TOKEN`, `..._READ_ONLY_TOKEN`, `..._URL`, `UPSTASH_KV_URL`, `UPSTASH_REDIS_URL`) regenerated |
...
| Set new Upstash token in Vercel env vars + local `.env.local` | ✅ Done 2026-05-04 — Vercel Marketplace integration auto-synced all 5 vars to Production + Preview + Development; local `.env.local` updated from `vercel env pull` and verified with `/ping` returning `PONG` |
