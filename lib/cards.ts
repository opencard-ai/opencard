...
export function getAllCards(): CreditCard[] {
  let files = fs.readdirSync(CARDS_DIR).filter((f) => f.endsWith(".json"));
  console.log(`[BUILD] Files in dir: ${files.length}`);
  if (files.length < 50) files = HARDCODED_CARDS.map(id => `${id}.json`);
